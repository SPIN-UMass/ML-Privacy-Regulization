...
r=pickle.load(open('./random_r_texas100'))
X=X[r]
Y=Y[r]
...
def save_checkpoint(state, is_best, checkpoint='./checkpoints_texas/', filename='checkpoint.pth.tar'):
...
for epoch in range(0, 20):
    ...
    if epoch <1: 
        ...
        for i in range(76):
            ...
            train_privatly(train_classifier_data_tensor,train_classifier_label_tensor, net,attack_model, criterion, optimizer, epoch, use_cuda,2,(2*i)%152,1.0)
...
resume='./checkpoints_texas/Depoch10'
...
print('Best classification acc:%.2f'%(final_test_acc))
