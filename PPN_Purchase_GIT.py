...
if not os.path.isfile(DATASET_FILE):
	...
	urllib.urlretrieve("https://www.comp.nus.edu.sg/~reza/files/dataset_purchase.tgz",os.path.join(DATASET_PATH,'tmp.tgz'))
...
len_train =len(X)
r=pickle.load(open('./random_r_purchase100'))
X=X[r]
Y=Y[r]
...
def train_privatly(train_data,labels, model,inference_model, criterion, optimizer, epoch, use_cuda,num_batchs=10000,skip_batch=0,alpha=0.5):
    ...
    for ind in xrange(skip_batch,len_t):
...
def save_checkpoint(state, is_best, checkpoint='./purchase_checkpoints', filename='checkpoint.pth.tar'):
...
resume='./purchase_checkpoints/Depoch13'
...
test_loss, final_test_acc = test(test_data_tensor,test_label_tensor, net, criterion, 0, use_cuda)
...
for epoch in range(0, epochs):
    ...
    test_loss, test_acc,mem,nonmem = test_attack(train_classifier_data_te_attack_tensor,train_classifier_label_te_attack_tensor
...
print('Best Attack Acc: %.2f'%(best_acc))
